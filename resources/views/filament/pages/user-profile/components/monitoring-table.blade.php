...
<div x-data="{ 
        ...
        <tbody>
            @forelse($user->monitoringKredit as $kredit)
                <tr class="border-b">
                    ...
                    <td class="py-2 px-3">
                        <button @click="showDetailModal = true; selectedRecord = {{ $kredit->load('buktiTindakan')->toJson() }}" 
    ...
    <div x-show="showDetailModal" 
        ...
        <!-- Modal Overlay -->
        <div class="fixed inset-0 bg-gray-900 bg-opacity-75" style="background-color:rgba(0, 0, 0, 0.75) !important;"></div>
                ...
                <div class="px-6 py-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex flex-col md:flex-row gap-4">
                                <div class="flex-1 bg-gray-50 p-3 rounded-lg">
                                    ...
                                    <p class="mt-1 text-sm text-gray-900" x-text="selectedRecord ? new Date(selectedRecord.created_at).toLocaleDateString('id-ID', {weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'}) : '-'"></p>
                                </div>
                                <div class="flex-1 bg-gray-50 p-3 rounded-lg">
                                    <label class="text-sm font-medium text-gray-500">Nomor Rekening</label>
                                    <p class="mt-1 text-sm text-gray-900" x-text="selectedRecord?.NOMOR_REKENING || '-'"></p>
                                </div>
                            </div>
                            <div class="flex flex-col md:flex-row gap-4">
                                <div class="flex-1 bg-gray-50 p-3 rounded-lg">
                                ...
                                <div class="flex-1 bg-gray-50 p-3 rounded-lg">
                                    ...
                                    <p class="mt-1 text-sm text-gray-900" x-text="selectedRecord ? new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(selectedRecord.PEMBAYARAN || 0) : '-'"></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p class="mt-1 text-sm text-gray-900 whitespace-pre-wrap" x-text="selectedRecord?.HASIL_TINDAKAN || '-'"></p>
                        </div>
                                ...
                                <div class="max-h-[200px] overflow-y-auto pr-2">
                                    <div class="flex flex-wrap gap-3">
                                        <template x-for="bukti in selectedRecord.bukti_tindakan" :key="bukti.id">
                                            <div class="relative group cursor-zoom-in w-24 h-24"
        ...
        <div class="fixed inset-0 z-[9998] flex items-center justify-center p-4">
            ...
            <div class="w-screen h-screen flex items-center justify-center p-4"
                ...
                <img :src="selectedImage ? '/storage/' + selectedImage : ''" 
                     class="max-h-screen max-w-screen w-auto h-auto object-contain"
                     alt="Preview Bukti Tindakan">
        </div>
    </div>
</div>
